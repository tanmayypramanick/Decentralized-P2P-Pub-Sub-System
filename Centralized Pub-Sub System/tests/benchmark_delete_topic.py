...
# Benchmark the delete_topic API for a peer node
async def benchmark_delete_topic(peer_ip, peer_port, num_topics):
    client = ClientAPI(peer_ip, peer_port)
    latencies = []
    start_time = time.time()

    # Pre-create topics to delete
    for _ in range(num_topics):
        topic_name = f"topic_{uuid.uuid4()}"
        print(f"[LOG] Pre-creating topic: {topic_name} on {peer_ip}:{peer_port}")
        await client.create_topic(topic_name)

        # Measure latency for each topic deletion
        topic_start_time = time.time()
        print(f"[LOG] Deleting topic: {topic_name} on {peer_ip}:{peer_port}")
        await client.delete_topic(topic_name)
    ...
    print(f"[LOG] Completed {num_topics} topic deletions on {peer_ip}:{peer_port}")
    ...
    with open(csv_filename, 'w', newline='') as file:
        ...
        for peer_count in range(1, num_peers + 1):
            ...
            for peer_id in range(peer_count):
                ...
                throughput, avg_latency = asyncio.run(benchmark_delete_topic(peer_ip, peer_port, num_topics))
