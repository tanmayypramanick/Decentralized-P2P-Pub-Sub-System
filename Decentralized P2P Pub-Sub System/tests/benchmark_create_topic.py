...
# Benchmark the create_topic API for a peer node with a longer timeout and delay
async def benchmark_create_topic(peer_id, num_topics, timeout=10, delay_between_requests=0.1):
    ...
    for _ in range(num_topics):
        ...
        try:
            print(f"[LOG] Creating topic: {topic_name} on peer ID: {peer_id}")
            await asyncio.wait_for(client.create_topic(topic_name), timeout=timeout)
            topic_end_time = time.time()
            latencies.append(topic_end_time - topic_start_time)
        except asyncio.TimeoutError:
            print(f"[ERROR] Timeout: Creating topic {topic_name} on peer ID {peer_id}")
    ...
    print(f"[LOG] Completed {num_topics} topic creations on peer ID: {peer_id}")
...
def run_create_topic_benchmark(num_peers, num_topics, csv_filename):
    ...
    with open(csv_filename, 'w', newline='') as file:
        ...
        for i in range(num_peers):
            ...
            time.sleep(2)  # Delay to ensure each peer starts properly
        ...
        for peer_count in range(1, num_peers + 1):
            ...
            for i in range(peer_count):
                ...
                throughput, avg_latency = asyncio.run(benchmark_create_topic(peer_id, num_topics))
